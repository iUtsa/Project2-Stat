...
public class HashMapper {
    ...
    static class DumbHashMap<K, V> {
        ...
        public List<Entry<K, V>> getBucketEntries(int index) {
            if (index < 0 || index >= size) {
    ...
    static class TextAnalyzer {
        ...
        // Generate a collision spectrum chart
        public static JFreeChart generateCollisionSpectrum(Map<String, Object> stats) {
        ...
        public static double calculateSimilarity(String text1, String text2, int mapSize) {
            ...
            double maxPossibleDistance = Math.sqrt(mapSize * Math.pow(
                    Math.max(
                            (int) stats1.get("uniqueWords"),
                            (int) stats2.get("uniqueWords")
                    ), 2));
    ...
    static class Plotter {
        ...
        public static JPanel create3DPlot(int[] bucketDistribution, int size) {
            ...
            for (int i = 0; i < size; i++) {
                for (int j = 0; j < size; j++) {
                    ...
                    // Create a surface by combining values
                    data[i][j] = Math.sqrt(value1 * value2) * Math.sin(i * j / (double) (size * size) * Math.PI);
                }
            }
